...
string strLogFilePathName = _T("");
	
class CLog    
{    
public:  
	static void SetLogFilePathName(string strFileName)
	{
		...
		strcat(szPath, "\\");  
		strcat(szPath, strFileName.c_str());
		strLogFilePathName = szPath;
		strLogFilePathName += _T(".log");
	}
    ...
    {  
       if(strLogFilePathName == _T(""))
		{	
			...
			fout.seekp(ios::end);  
			fout << GetSystemTime() << x <<" = "<<x2<<endl;  
			fout.close(); 
		}
		else
		{
			ofstream fout(strLogFilePathName, ios::app);  
			fout.seekp(ios::end);  
			fout << GetSystemTime() << x <<" = "<<x2<<endl;  
			fout.close(); 
		}			
...
//用下面这些宏来使用本文件
#define SETLOGFILEPATHNAME(x)   CLog::SetLogFilePathName(x)
